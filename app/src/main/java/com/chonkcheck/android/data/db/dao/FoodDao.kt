...
interface FoodDao {
    ...
    @Query("SELECT * FROM foods WHERE id = :foodId AND deletedAt IS NULL")
    fun getFoodById(foodId: String): Flow<FoodEntity?>

    @Query("SELECT * FROM foods WHERE id = :foodId AND deletedAt IS NULL")
    suspend fun getFoodByIdOnce(foodId: String): FoodEntity?

    @Query("SELECT * FROM foods WHERE barcode = :barcode AND deletedAt IS NULL LIMIT 1")
    ...
    @Query("SELECT * FROM foods WHERE deletedAt IS NULL ORDER BY updatedAt DESC LIMIT :limit")
    ...
    @Query("UPDATE foods SET deletedAt = :deletedAt WHERE id = :foodId")
    suspend fun softDelete(foodId: String, deletedAt: Long = System.currentTimeMillis())
    ...
    @Query("SELECT COUNT(*) FROM foods WHERE deletedAt IS NULL")
